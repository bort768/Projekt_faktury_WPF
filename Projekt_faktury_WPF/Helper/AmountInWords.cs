...
namespace Projekt_faktury_WPF.Helper
{
    public class AmountInWords
    {
        ...
        public static string KwotaSlownie(double kwota, string kodWaluty)
        {
            int calosc = (int)kwota;
            int ulamki = (int)(kwota * 100) % 100;
            return string.Format("{0} {1}, {2} {3}",
                LiczbaSlownie(calosc),
                WalutaSlownie(calosc, kodWaluty),
                LiczbaSlownie(ulamki),
                WalutaSlownie(ulamki, "." + kodWaluty));
        }
